;(function () {
	...
	const legalAnimate = function() {

		const legal = $('#fh5co-legal');
		if ( legal.length > 0 ) {	
	...
	const doneAnimate = function() {
		const done = $('#fh5co-done');
		if ( done.length > 0 ) {	

			done.waypoint( function( direction ) {
										
				if( direction === 'down' && !$(this.element).hasClass('animated') ) {


					var sec = done.find('.to-animate').length,
						sec = parseInt((sec * 200) + 400);

					setTimeout(function() {
						done.find('.to-animate').each(function( k ) {
							var el = $(this);
